...
// .wrangler/tmp/bundle-29HqbC/checked-fetch.js
...
// .wrangler/tmp/bundle-29HqbC/strip-cf-connecting-ip-header.js
...
async function createRoom(request, env, headers) {
  try {
    ...
    await env.DB.prepare(
      "INSERT INTO rooms (id, host_id, created_at, is_public, participant_count, chat_mode) VALUES (?, ?, ?, ?, 0, ?)"
    ).bind(roomCode, "pending", Date.now(), isPublic ? 1 : 0, "free").run();
    return Response.json({ roomCode, isPublic }, { headers });
  } catch (e) {
    return Response.json({ error: "Failed to create room" }, { status: 500, headers });
  }
}
__name(createRoom, "createRoom");
async function listPublicRooms(env, headers) {
  try {
    const result = await env.DB.prepare(
      "SELECT id, host_id, created_at, participant_count FROM rooms WHERE is_public = 1 ORDER BY created_at DESC LIMIT 20"
    ).all();
    ...
    return Response.json(rooms, { headers });
  } catch (e) {
    return Response.json([], { headers });
  }
}
__name(listPublicRooms, "listPublicRooms");
...
// .wrangler/tmp/bundle-29HqbC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-29HqbC/middleware-loader.entry.ts
